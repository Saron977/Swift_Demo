...
// MARK: 设置颜色
/// 设置颜色值
public let Color_HexRGB:((Int) -> UIColor) = { (rgbValue : Int) -> UIColor in
    return Color_HexRGBAlpha(rgbValue,1.0)
}

/// 通过 十六进制与alpha来设置颜色值  （ 样式： 0xff00ff ）
public let Color_HexRGBAlpha:((Int,Float) -> UIColor) = { (rgbValue : Int, alpha : Float) -> UIColor in
...
public let Color_RGBAlpa:((Float,Float,Float,Float) -> UIColor ) = { (r: Float, g: Float , b: Float , a: Float ) -> UIColor in
